...
async function getEntries(req, res) {
  try {
    ...
    const entries = await Entry.find({ userId: req.user.userId, date }).sort({ createdAt: -1 });
    ...
    const summary = {
      calories: {
        ...
        netCalories: calorieIntake - caloriesBurned,
      },
      macros: {
...
async function analyzeText(req, res) {
  try {
    ...
    if (error.name === 'ValidationError') {
      return res.status(400).json({ success: false, error: 'Database validation error: ' + error.message });
    }
...
async function deleteEntries(req, res) {
  ...
  try {
    if (!id && !date) {
      return res.status(400).json({ error: 'Either id or date is required' });
    }
    ...
    if (date) {
      ...
      if (!year || !month || !day || day < 1 || day > 31 || month < 1 || month > 12) {
        return res.status(400).json({ error: 'Invalid date format. Use YY-MM-DD' });
      }
...
async function analyzeMealImageHandler(req, res) {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
    ...
    if (error.message.includes('Only image files')) {
      return res.status(400).json({ success: false, error: 'Invalid file type. Only image files are allowed.' });
    }
